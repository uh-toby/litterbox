...
The kit installs the CLIs during `setup.install` and provides egress, pi defaults, and skills. The only host-side step is storing the Sentry, Linear, Buildkite, and OpenRouter secrets; GitHub needs nothing, see below. OpenRouter is supplied by sbx's built-in shell-agent credential rather than declared by this kit, avoiding a credential collision when the kit is composed with `shell`.

Authored against kit-spec **v2**, validated on sbx `v0.38.0`.

## What it does

| Concern | How |
| --- | --- |
| CLIs | `setup.install` installs pi 0.82.1 via npm and downloads `sentry` 0.38.0, `linear` 2.3.0, `gh` 2.96.0 and `bk` 3.44.1 to `/usr/local/bin` |
...
| Egress | `permissions.network.allow` opens the API hosts plus the release-download hosts |
...
> `sbx secret set --help` only advertises its built-in services, but arbitrary service names are accepted.
...
```sh
sbx run --kit ~/.config/sbx/kits/toby-sbx claude .
```
...
```sh
sbx run --kit ~/.config/sbx/kits/toby-sbx shell .
```
...
created to run the setup commands and receive the seeded pi configuration.

## What was verified

Confirmed in real sandboxes, on both the `shell` and `claude` agents:

- All four CLIs install and run during `setup.install`: `sentry` `0.38.0`, `linear` `2.3.0`, `gh` `2.96.0`, `bk` `3.44.1` (at `/usr/local/bin/bk`). The `xz-utils` fallback works — also checked standalone in `debian:bookworm-slim` with only `curl` present.
...
- **Use `--kit` at create time, not `sbx kit add`.** Setup commands run when the sandbox is created; adding a kit later does not replay them unless the sandbox is recreated.
...
- **Pinned versions.** `0.82.1`, `0.38.0`, `v2.3.0`, `v2.96.0` and `v3.44.1` are hardcoded in `spec.yaml`. Bump them there when you update the tools; nothing auto-updates.
